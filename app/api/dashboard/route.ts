...
export async function GET() {
  try {
    ...
    const studyEntryIds = [...new Set((attempts || []).map((a) => a.study_entry_id).filter(Boolean))]
    ...
    if (studyEntryIds.length > 0) {
      ...
      if (entriesError) {
        return NextResponse.json({ error: 'Failed to load related study entries.', details: entriesError.message }, { status: 500 })
      }
    ...
    const mergedAttempts = (attempts || []).map((attempt) => ({
      ...attempt,
      study_entries: attempt.study_entry_id ? entriesMap[attempt.study_entry_id] || null : null,
    }))

    const totalAttempts = mergedAttempts.length
    const averageScore = totalAttempts > 0
      ? Math.round(mergedAttempts.reduce((sum, item) => sum + (item.correctness_score || 0), 0) / totalAttempts)
      : 0

    const exactMatches = mergedAttempts.filter((item) => item.feedback_json?.status === 'Exact match').length
    const termAttempts = mergedAttempts.filter((item) => (item.feedback_json?.practice_type || 'term') === 'term').length
    const sentenceAttempts = mergedAttempts.filter((item) => item.feedback_json?.practice_type === 'sentence').length

    const streak = calculateStreak(
      mergedAttempts.map((item) => item.created_at).filter((value): value is string => Boolean(value))
    )
    ...
    return NextResponse.json({
      success: true,
      summary: { totalAttempts, termAttempts, sentenceAttempts, averageScore, exactMatches, streak },
      recentTerms, smartReviewTerms,
    })
  } catch (error) {
    return NextResponse.json({ error: 'Failed to load dashboard data.', details: error instanceof Error ? error.message : 'Unknown error' }, { status: 500 })
  }
}
